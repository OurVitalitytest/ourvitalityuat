...
{
    public partial class ucProgressBar : System.Web.UI.UserControl
    {
        ...
        {
            if (!String.IsNullOrEmpty(Convert.ToString(Session["total_calories_targeted_to_burn_today"])))
            {

                decimal intBlocks = Convert.ToDecimal(Session["total_calories_burn_today"]);
                decimal totalToBurn = Convert.ToDecimal(Session["total_calories_targeted_to_burn_today"]);

                // add a new row to the table
                _tblBlock = new TableRow();
                // create cells and add to the row
                decimal K = Math.Ceiling((Convert.ToDecimal(intBlocks) * 100 / totalToBurn));
